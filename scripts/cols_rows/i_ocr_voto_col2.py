...
def extract_page_from_dirname(dir_name):
    """
    Extrae el número de página del dir_name.
    Ejemplo: '000058a7-4618-53af-82f8-13266eef3ace_page003_' -> 'page003'
    """
...
def process_images():
    ...
    # Los JSONs se guardan con el dir_name completo
    existing_jsons = set()
    ...
    # Filtrar dir_names que ya existen
    dir_names_df_filtered = dir_names_df[~dir_names_df["dir_name"].isin(existing_jsons)]
    ...
    print("\n🔍 4. Agrupando datos por dir_name...")
    documents = defaultdict(list)

    # Agrupar las filas por dir_name
    for _, row in df_filtered.iterrows():
        dir_name = row["dir_name"]
        image_path = row["image_path"]
        image_name = row["image_name"]

        documents[dir_name].append((image_path, image_name))

    print(f"✓ Se encontraron {len(documents)} documentos únicos (dir_names)")
    ...
    with tqdm(total=total_images, desc="Procesando imágenes", unit="img") as pbar:
        for dir_name, images in documents.items():
            # Extraer el número de página del dir_name
            page = extract_page_from_dirname(dir_name)
            ...
            # Crear estructura del JSON con meta y votos
            doc_result = {"meta": page, "votos": votos}

            # Guardar JSON del documento con el dir_name completo
